   ...
   */
    ...
    function openAt(pantry, when) {
        ...
        for (var i = -1, len = hours.length; ++i < len;) {
            ...
            //  repeatsOn: (day name)
            //
            // It needs to handle more complex cases: e.g., every first
            // and third Saturday
        ...
        return {
            open: false,
            next: nextOpen && nextOpen.getTime()
        };
    }
  ...
  app.get('/api/pantry/search', function(req, res) {
      if ('development' == app.get('env'))
          var search = function(params) {
              var d = params.when ? new Date(params.when) : new Date();
      ...
      query.radius = parseFloat(query.radius);
      query.when = parseInt(query.when);

      if (isNaN(query.radius))
          delete query.radius;
